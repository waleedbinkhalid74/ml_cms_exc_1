...
class Pedestrian:
    """
    Pedestrian class represents a 2D array of cells.
    """
    _id_counter = 0
    ...
    def is_valid(self):
        ...
        """
        return self.cell.cell_type == CellType.OCCUPIED
...
class Grid:
    ...
    def __init__(self, rows, cols):
        ...
        self.pedestrians = []
        self.targets = []
        self.cells = [[Cell(row, column) for column in range(cols)] for row in range(rows)]
    ...
    def is_valid(self):
        ...
        for i in range(len(self.pedestrians)):
            for j in range(i, len(self.pedestrians)):
